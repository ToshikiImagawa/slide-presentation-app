...
| **書き出し時の選択** | 編集モードの **同梱アドオン** チェックボックス（および `npm run export:slides --addons a,b`） | `.tgz` 書き出し時に同梱するアドオンを選択します                                                                              |
| **組み込みの増減** | 編集モードの **組み込みアドオン (dev)** パネル（開発ビルドのみ）                          | `addons/src/<名前>/entry.ts` を生成 / 削除します。変更後は `npm run build:addons` を実行して再ビルドしてください             |

### AI 生成

編集モードの **AI 生成** パネルで、プロンプトからスライド（`slides.json`）を生成できます。生成結果は器の単一真実源へ
**全体置換**で流し込まれ、そのままプレビュー・手動編集・保存・書き出しに繋がります。生成方式は 2 つから選べます。
