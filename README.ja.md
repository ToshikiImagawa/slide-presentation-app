...
| **.tgz 書き出し** | `.tgz` パッケージを生成します（名前 / バージョンはツールバーの入力欄から）。参照アセットを同梱し、**ファイルを開く** と往復できます |
...
| **書き出し時の選択** | 編集モードの **同梱アドオン** チェックボックス（および `npm run export:slides --addons a,b`） | `.tgz` 書き出し時に同梱するアドオンを選択します。候補はパッケージのアドオンと dev の組み込みアドオンの和集合で、候補が無くても一覧は消えず「同梱できるアドオンがありません」を表示します |
