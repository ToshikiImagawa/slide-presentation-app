...
本アプリは自己署名のみで Apple の公証(notarization)を受けていないため、初回起動時に macOS Gatekeeper が
「開発元を検証できません」という警告を表示します。Finder でアプリを右クリック(または Control キーを押しながらクリック)
して「開く」を選び、表示されるダイアログで確認すると quarantine 属性が解除され、以降この警告は表示されなくなります。
これは macOS 自体によるコード署名検証です。
...
保存されます。インストールや起動のたびにキーチェーンのパスワードを求められることはありません。上記の Gatekeeper 警告が
表示されうる唯一のダイアログです。
...
プレゼンテーション中に `?` を押すとアプリ内に一覧を表示できます（**設定 → キーボードショートカット → 表示** からも同じ
一覧を開けます）。
