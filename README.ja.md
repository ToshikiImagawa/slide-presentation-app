...
| 方式                       | 前提                                                       | 課金 / オンライン依存                                                             |
|----------------------------|------------------------------------------------------------|----------------------------------------------------------------------------------|
| **内蔵（Anthropic API）**  | Anthropic の API キーを登録（既定モデル `claude-opus-4-8`）| オンライン接続が必要で、API キーの利用量に応じて課金されます                      |
| **外部（Claude Code CLI）**| ローカルに `claude` コマンドがインストール済み             | お使いの Claude の契約・利用条件に従います（別途 API キーは不要）                 |

- **API キーの設定 / 削除** — 内蔵方式のパネルでキーを入力して **保存**、不要になれば **削除** します。キーは OS の
  キーチェーン（macOS Keychain / Windows Credential Manager / Linux Secret Service）に保管され、Web 層へは一切渡しません。
  ネットワーク通信も含め、キーの読み書きと API 呼び出しはすべて Rust 境界に閉じています（最小権限）。
- **事前ゲート** — 内蔵はキー未登録時、外部は `claude` 未検出時に生成ボタンを無効化し、設定導線を示します。
...
生成・キー操作・ネットワークは編集モード かつ 生成有効時のみ有効化されます（閲覧・発表本番では到達しません）。
