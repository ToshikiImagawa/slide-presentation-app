...
アプリを右クリックして開く Gatekeeper 回避手順が廃止されたため、ダウンロードした `.dmg`（中のアプリではなく
`.dmg` 自体）が Gatekeeper にブロックされ、「Apple はマルウェアが含まれていないことを検証できませんでした」
という警告が表示されます。これは初回だけでなく、**新しいバージョンをダウンロードするたびに**発生します。
...
- **進捗と中断** — 生成中は進捗を表示し、**中断** で実行中の生成を止められます。失敗・中断・オフライン時は器の内容を
  壊さず、手動編集に安全に戻れます。
- **見た目をチェックして修正** — **生成** の隣にあるこのボタンを押すと、サンプル/基準見本デッキの検査で使っている
  見た目のチェック（はみ出し・セーフエリア侵入・装飾との重なり・埋める要素の高さ 0・内部クリッピング）を全スライドに
  一括で実行します。問題が見つかった場合だけ、上記の自動修正ループと同じ経路でAIに修正を依頼します（文言・構成の
  調整のみを指示し、レイアウトの実装は変更しません）。結果は同じ差分確認ダイアログで確認できます。
...
| `only`    | object | -              | ロゴを描画するスライドの絞り込み              |

`meta.logo` を省略するとロゴは表示されません。`width` と `height` を省略すると、それぞれ既定値の `120` と `40` が使われ
ます。既定ではロゴは全スライドの左下に表示されますが、`anchor`/`offset` を指定すれば別の位置に配置できます。`offset`
の既定値は `bottom-left` 専用の値なので、他の `anchor` を使う場合は必ず `offset` を指定してください（指定しないと
画面外に出て見えなくなります）。
...
| `anchor`    | string | `top-right`      | 9方向のいずれか（例: `bottom-left`、`center`）                |
| `offset`    | object | `{x:-30,y:20}`   | アンカーからのpxオフセット `{ x, y }`                         |
...
`meta.confidential` を省略すると透かしは表示されません。必須フィールドは `text` のみです。既定位置はロゴの既定位置
（左下）とあえて逆の右上にしており、ロゴと透かしを両方、位置を指定せずに使っても重なりません。`offset` の既定値は
`top-right` 専用の値なので、他の `anchor`（例: `bottom-left`）を使う場合は必ず `offset` を指定してください（指定
しないと画面外に出て見えなくなります）。
...
左上の歯車アイコン（設定ボタン）をクリックすると設定ウィンドウが開き、言語を選択できます。ホーム画面・プレゼンテーション
画面・編集画面のいずれからも開けるため、初回起動直後にスライドを開かずに言語を切り替えられます。同じ言語設定が発表者
ビューにも適用されます。
...
| ボタン               | アイコン | 機能                                                      |
|----------------------|----------|-----------------------------------------------------------|
| 再生 / 一時停止 / 再開 | スピーカー | 現在のスライドの音声をクリックごとに再生 / 一時停止 / 再開 |
...
![ツールバー — 左: ホーム / 編集 / ツールバーを隠す / 設定、右: 音声再生 / 自動再生 / 自動スライドショー / PDF書き出し / 録画 / 発表者ビュー](resources/screenshots/ja/toolbar.png)

### 手動再生

スピーカーアイコンをクリックすると現在のスライドの音声が再生されます。もう一度クリックすると一時停止し、再度クリック
すると再開します。`voice` が定義されていないスライドではアイコンは表示されません。
...
右上ツールバーの **PDFで保存** ボタンをクリックすると、現在のデッキを PDF として書き出せます（1 スライド 1
ページ）。保存先はネイティブの保存ダイアログで選びます。書き出し中はボタンがビジー状態になり、クリックできなくなり
ます。

## 録画

右上ツールバーの録画ボタン（**PDFで保存** の隣）をクリックすると、選択した画面/ウィンドウの共有と現在のスライドの
スピーカーノート音声（voice再生音）をまとめて録画し、動画ファイルとして保存できます。

- クリックするとOSの画面/ウィンドウ共有選択ダイアログが開き、選択が完了すると録画が始まります。共有選択をキャンセル
  した場合、ボタンは操作前の状態のままです。
- 録画中はボタンの見た目が変化し、録画中であることが分かります。自動再生・自動スライドショーと併用できるため、
  発表者が操作しなくても完全自動のプレゼンテーションをそのまま録画できます。
- 再度ボタンを押すと録画が停止し、ネイティブの保存ダイアログで動画ファイルの保存先を選べます（`.mp4`。MP4録画に
  対応していないWebViewでは `.webm` にフォールバックします）。
- 録画中に共有対象が失われたりエラーが発生した場合も、プレゼンテーション本体を中断せずに録画を安全に終了し、
  記録済みの内容を保存できる状態にします。

> **macOSでの注意**: 画面録画には **システム設定 → プライバシーとセキュリティ** の **画面録画** 権限が必要です。
> アプリの `Info.plist` にはカメラ・マイクの使用目的の説明が含まれていますが、これは WKWebView が
> `navigator.mediaDevices`（`getDisplayMedia` を含む）を公開するための要件を満たすためだけのものであり、
> アプリ自体はカメラやマイクに一切アクセスしません。
...
>
> パッケージの `addons/manifest.json` で宣言され、かつ `addons/` 配下にあるアドオンのみが読み込まれます。 <!-- doc-check-ignore -->
