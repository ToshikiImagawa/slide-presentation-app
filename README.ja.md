...
これは macOS 自体によるコード署名検証であり、アプリがパスワードを要求しているわけではありません。
...
本アプリは現状、アプリ内で更新を自動チェックする機能を搭載していません。リリースパイプラインは署名済みの
`latest.json` 更新マニフェストを生成・配信しており、Tauri updater プラグインもネイティブ側には登録済みですが、
フロントエンドからそれを呼び出して更新を確認・適用する処理はまだ実装されていません。現状は
[Releases](https://github.com/ToshikiImagawa/slide-presentation-app/releases) から最新のインストーラをダウンロードし、
既存のアプリに上書きインストールしてください。将来アプリ内更新チェックを実装した場合、更新は同じ署名鍵で検証され、
環境によっては更新後に再度上記の Gatekeeper 警告が表示される可能性があります。
