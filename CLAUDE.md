...
- **自動アップデート**（#121・v2.3.1で配信方式を変更）: 起動時に 1 回だけ更新を確認する。`src-tauri/src/update_check.rs` の `check_for_update`/`install_update` が静的URL `releases/latest/download/latest.json` を updater の `endpoints` に設定して `check()` する。**当初は GitHub API（`GET /repos/{owner}/{repo}/releases/latest` → `assets[]` から `id` を探索 → `/repos/{owner}/{repo}/releases/assets/{id}`）でアセットURLを解決していたが、未認証アクセスは IP あたり60リクエスト/時までしかなく、同一ネットワークの他ユーザー（自分のツール呼び出しを含む）と枠を共有して枯渇し、リリース直後に更新検出できない事象が発生した（v2.3.1）。静的URLは `api.github.com` とは別カウンタ（`github.com` 側のリダイレクト→署名付きCDN URLで、レート制限ヘッダも付かない）で、両方式とも常に `releases/latest`（prerelease除外）を見る点で挙動が同じだったため、レート制限を受けない静的URLに一本化した（API経由でのプレリリース配信制御は将来必要になった時点で再検討する）**。`check_for_update` は開発ビルド（`cfg!(debug_assertions)`。`npm run tauri:dev` 等）では常にチェックせず `None` を返す（他の dev 限定機能と同じ規約）。また plugin-store（`update-check.json` の `lastCheckedAtEpochSecs`）に前回チェック時刻を記録し、24時間（`CHECK_COOLDOWN_SECS`）以内の再起動では実際には確認しない（フリート全体での呼び出し回数を間引く。`install_update` はユーザーの明示操作なのでこのクールダウン・dev ガードの対象外）。**`tauri.conf.json` の `plugins.updater.endpoints` は JSON にコメントを書けないため、プラグイン初期化時の静的スキーマ要件を満たすだけの値で、実行時は必ず `update_check.rs` が解決した URL で上書きされる（真実源はここ）**。フロントは `src/update.ts`（`check_for_update` / `install_update` を invoke する薄い層）、`useUpdateCheck`（`src/hooks/useUpdateCheck.ts`。起動時チェック・ダイアログ開閉・インストール実行の状態を持つ）、`UpdateDialog`（`src/components/UpdateDialog.tsx`。画面遷移で閉じる＝発表・編集中に割り込まない）のみ。オフライン・`latest.json` 未添付はすべて Err にし、フロント側は無言で諦める（利用を妨げない）。`@tauri-apps/plugin-updater` の JS API は動的 endpoints に対応できないため導入していない（Rust 側の独自コマンドのみを使う）
- **macOS の追加 Info.plist**（#381）: `src-tauri/Info.plist` は `tauri.conf.json` と同じディレクトリにあるため Tauri バンドラが既定の Info.plist へ自動マージする（`bundle.macOS.infoPlist` での明示は不要）。カメラ・マイクは一切使用しないが、WKWebView は `NSCameraUsageDescription`/`NSMicrophoneUsageDescription` のいずれも無いと `navigator.mediaDevices` 自体を非公開にする（`getDisplayMedia` を含む）ため、録画機能の成立にこの 2 キーが必須。`npm run tauri:dev` は非バンドルの生バイナリを実行するためこの制約を受けず、dev では再現しない（`.sdd/specification/presentation-recording_design.md` の 9.1 参照）
