...
- `draft`: 記述内容が確定していない、または対応する PRD がまだ `approved` でない
- `approved`: 記述内容がレビューを通過し、対応する PRD の `status: approved` および実装状況（`xxx_design.md` の `impl-status` および実装進捗表）と整合していることを確認済み

実装の完了度は `xxx_design.md` の `impl-status` フィールドで別軸管理するため、`status: approved` かどうかと実装が完了しているかどうかは独立に判断してください。
...
# 9. 原則への言及 `<RECOMMENDED>`

本仕様が [CONSTITUTION.md](../CONSTITUTION.md) のどの原則に基づいた設計かを明記します。

| 原則ID | 原則名 | 本仕様での適用内容 |
|------|------|------------|
| [A-003] | [データ駆動型スライドアーキテクチャ] | [適用内容] |
