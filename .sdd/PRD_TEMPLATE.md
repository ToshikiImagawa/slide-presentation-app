...
## front matter の `status` 運用規約

`status` は各ドキュメントの**記述内容の承認状態**を表します。

- `draft`: 要求内容がレビュー中で確定していない
- `approved`: 要求内容がレビューを通過し確定した

PRD が `approved` になった後、対応する `xxx_spec.md` / `xxx_design.md` の記述内容が実装（`xxx_design.md` の `impl-status` および実装進捗表）と整合していることを確認できたら、それらの `status` も `approved` に更新してください。実装の完了度は `impl-status` フィールドで別軸管理するため、承認判断とは独立に扱います。
