---
id: prd-speaker-note-audio
title: スピーカーノート音声再生（Speaker Note Audio）要求仕様書
type: prd
status: draft
priority: high
risk: high
...
- スライドコンテンツ内で参照されるコンポーネントは ComponentRegistry で管理し、拡張性を確保すること（A-004 準拠）。ただし、スピーカーアイコン（AudioPlayButton）・自動再生コントロールバー（AudioControlBar）はスライドコンテンツではなくアプリのクロム UI であり、App.tsx で直接構成するため A-004（ComponentRegistry 管理）の対象外とする
