...
本 PRD は、この生成フローをアプリに取り込む。具体的には (1) 編集モード内の生成パネルからのプロンプト入力と生成実行、(2) 内蔵（Anthropic API）と外部（Claude Code 等）を切り替えられる**差し込み可能な共通生成インターフェース**、(3) 生成結果 `slides.json` の**器への流し込み**（本番同一レンダラのライブプレビュー＋手動調整）、(4) 生成結果の**バリデーションと自動修正ループ**、(5) **API キーの安全な保管**、(6) ネットワーク通信と秘密情報を**編集モード かつ 生成有効時のみ**有効化する capability 分離、を定義する。
...
編集・保存・書き出し・ライブプレビュー・シリアライズ（`SlideEditor` / ライブプレビュー / 無損失往復 / Rust コマンド境界での書き込みゲート）は [slide-edit-mode.md](./slide-edit-mode.md) が定義済みであり、本 Feature はこれらを**再実装せず再利用**する。本 Feature が新たに導入するのは「生成」「ネットワーク通信」「秘密情報（API キー）の保管」であり、これらはビューワーの読み取り中心の性質と構造的に分離する。
...
- 生成を内蔵するにはネットワーク通信と API キーという新しい関心事が加わるが、ビューワーは読み取り中心で外部通信も秘密情報も持たない設計である。**安全性を損なわずに生成能力だけを足す**方法が必要。
...
- **外部/内蔵の切替による柔軟性**: 差し込み可能な生成インターフェースにより、内蔵（Anthropic API）と外部（Claude Code 等）を用途・環境に応じて切り替えられる。

#### 方針（確定事項）

- **器を土台にする。** 生成は編集モードの一入力手段として `SlideEditor` の単一真実源へ流し込み、既存のライブプレビュー・保存・書き出しをそのまま再利用する。
- **生成部は差し込み可能なインターフェースにする。** 内蔵（Anthropic API）と外部（Claude Code 等）を編集モード内で切り替え可能とする。
- **ネットワーク・秘密情報はネイティブ（Rust）側に閉じる。** API キーは OS キーチェーンに保管し、HTTP 実行も Rust コマンド境界に集約する。生値・fetch 権限をフロントエンド（JS）へ渡さない。
...
| Anthropic API | 内蔵生成の呼び出し先となる外部 LLM サービス（オンライン・従量課金）                          |
...
| 生成方式の切替                 | 差し込み可能な共通生成インターフェースを介して、内蔵（Anthropic API）と外部（Claude Code 等）を切り替える |
| API キーの設定・更新・削除        | Anthropic API キーを OS キーチェーンに安全に保管し、設定・更新・削除する                        |
...
- 差し込み可能な生成インターフェース（#14）
    - 内蔵（Anthropic API）と外部（Claude Code 等）を切り替える共通インターフェース
    - 内蔵生成は Rust コマンド境界から Anthropic Messages API（既定 `claude-opus-4-8`）を呼び出す
- 生成結果の器への流し込み（#14）
...
- API キーの安全な保管（#14）
    - Anthropic API キーの OS キーチェーンへの保管・更新・削除
...
requirementDiagram
    ...
    functionalRequirement BuiltinAnthropic {
        id: FR_003
        text: "内蔵生成は Rust コマンド境界から Anthropic Messages API を既定モデルで呼び出すこと"
        risk: high
        verifymethod: test
    }
    ...
    functionalRequirement KeyStorage {
        id: FR_006
        text: "Anthropic API キーを OS キーチェーンに保管し設定更新削除でき生値を WebView へ渡さないこと"
        risk: high
        verifymethod: inspection
    }
    ...
    functionalRequirement PreGate {
        id: FR_007
        text: "内蔵はAPIキー未設定 外部は対応CLI未検出の前提未充足時は生成導線を無効化し設定へ誘導すること 判定条件は設計で列挙する"
        risk: medium
        verifymethod: demonstration
    }
    ...
    designConstraint NativeBoundary {
        id: DC_002
        text: "ネットワーク実行と API キー保管を Rust コマンド境界に集約し内蔵はネイティブHTTPで接続し WebView から直接 fetch しないこと"
        risk: high
        verifymethod: inspection
    }
    ...
    GenerationFoundation - contains -> BuiltinAnthropic
    ...
    BuiltinAnthropic - traces -> SecretMinimization
    ...
    BuiltinAnthropic - traces -> NativeBoundary
...
生成に伴って新たに導入されるネットワーク通信と秘密情報（API キー）が、ビューワーの読み取り中心の安全性を損なわないこと。ネットワークと秘密情報は編集モード かつ 生成有効時のみ有効化され、通信実行とキー保管はネイティブ（Rust）側に閉じ、フロントエンド（JS）へ fetch 権限や秘密の生値を開放しないこと。
...
生成は、内蔵（Anthropic API）と外部（Claude Code 等）を差し替え可能な**共通の生成インターフェース**を介して行う。抽象の切れ目は「モデル呼び出し」ではなく「生成器（プロンプト → `slides.json` を返す単位）」に置き、外部の完結した生成手段（Claude Code 等）も同一の契約を満たせるようにする。利用者は編集モード内で生成方式を切り替えられる。

**検証方法:** デモンストレーション（内蔵/外部の切替が機能する）による検証

### FR-003: 内蔵生成（Anthropic API 呼び出し）

**優先度**: Must ／ **派生元**: UR-001 / UR-002

内蔵生成は、Rust コマンド境界から Anthropic Messages API を呼び出して `slides.json` を生成する。使用モデルは実装時点の最新モデル（既定は `claude-opus-4-8`）とし、既定値＋利用者設定による上書き＋将来のキュレート選択の三段で解決する。呼び出しはネイティブ（Rust）側で行い、API キーの生値やネットワーク権限をフロントエンド（JS）へ渡さない（DC-002）。外部へ送信する情報はプロンプト・スキーマ／テンプレート・現行 `slides.json` に限定する（NFR-004）。
...
Anthropic API キーを OS キーチェーンに安全に保管し、アプリ内で設定・更新・削除できる。キーの生値はフロントエンド（WebView）へ渡さない。取得系はキーの生値を返さず、「登録済み／未登録」等の状態のみを返す。保管・取得・削除はネイティブ（Rust）コマンド境界に集約する（DC-002）。
...
生成の前提が満たされないときは、対応する生成導線を無効化し、設定への導線を提示する。対象は少なくとも次を含む: 内蔵生成の API キー未設定、外部生成の CLI 未検出（判定条件の完全な列挙は設計フェーズで確定する）。生成不可の状態を実行前に利用者へ明示し、無効な状態での生成実行を未然に防ぐ。
...
生成・ネットワーク通信・API キー操作は、**編集モード かつ 生成有効時のみ**実行可能とし、Rust コマンド境界でゲートする。編集モードが無効、または生成が無効なときは、生成・ネットワーク・キー操作コマンドを拒否する。フロントエンドの表示制御だけに依存せず、ネイティブ側でゲート条件を検査してからネットワーク／キーチェーンに到達する（DC-003）。
...
ネットワーク通信と秘密情報（API キー）を、Rust コマンド境界の単一チョークポイントに集約すること。`fetch` 等のネットワーク権限や秘密の生値をフロントエンド（JS）へ開放しないこと。生成無効・編集モード外では、ネットワークにもキーチェーンにも到達しないこと。
...
生成のために外部へ送信する情報は、次に限定すること: プロンプト、`slides.json` のスキーマ／テンプレート、（既存スライドを起点に生成する場合の）現行 `slides.json`。API キーの生値・任意のローカルファイル・他パッケージの内容を送信しないこと。送信情報・送信禁止情報を設計で明示し、コードレベルで検証可能とすること。
...
ネットワーク通信（Anthropic API 呼び出し）と API キー保管は、Rust コマンド境界の単一チョークポイントに集約する。内蔵生成はネイティブ（Rust）側の HTTP 経路で Anthropic API に直接（API キー方式）接続し、WebView からの直接 `fetch` は行わない。`@tauri-apps` の HTTP／秘密系権限や API キーの生値をフロントエンド（JS）へ開放しない。これは [slide-edit-mode.md](./slide-edit-mode.md) の DC-002（書き込みの Rust コマンド境界集約）を、ネットワークと秘密情報の領域へ拡張したものである。

**検証方法:** インスペクションによる検証

### DC-003: 生成ゲート（編集モード＋生成有効）

生成・ネットワーク通信・キー操作は、編集モード状態と生成有効フラグで実行時にゲートする。非有効時はコマンドを拒否する。ゲート検査はネイティブ（Rust）側で行い、ネットワーク／キーチェーンに到達する前に条件を満たすことを保証する。フロントエンドの表示制御だけに依存しない。
...
- ネットワーク通信は Rust 側の HTTP 経路で行い、API キー保管は OS キーチェーンを用いる。使用ライブラリ（HTTP クライアント・キーチェーンクレット）の選定は設計フェーズで確定する（DC-002）。
...
- 使用する Claude モデルは実装時点の最新（既定 `claude-opus-4-8`）とし、モデル ID は設定値として解決する（未設定時は既定へフォールバック）。

## 5.2. ビジネス的制約

- プレゼンテーションの表示品質・伝達力に影響を与えない（B-001）。
- 生成失敗・オフライン・キー未設定時も、既存のスライド表示・「開く」・編集モードの手動編集・保存・書き出しは従来どおり可能であること（A-005: フォールバックファースト設計）。
- **課金・オンライン依存は方式別に UI で周知する**（内蔵: Anthropic API へオンライン接続し、利用者の API キーで従量課金が発生する／外部: ローカル CLI 等・Claude Code の導入とログインが前提）。コスト上限管理はスコープ外とし、UI の注意書きで周知するに留める（ただし応答タイムアウト・自動修正試行上限でコストを境界付ける: NFR-005）。
...
- 内蔵生成には Anthropic API キーとオンライン接続、外部生成には対応する外部生成手段（Claude Code 等）の導入が、利用者環境に用意されること。
...
| 用語                    | 定義                                                                            |
|:----------------------|:------------------------------------------------------------------------------|
| 内蔵生成                 | アプリが Anthropic API を直接呼び出して `slides.json` を生成する方式                        |
...
| OS キーチェーン           | API キー等の秘密情報を OS のセキュアストレージ（macOS Keychain / Windows Credential Manager 等）に保管する仕組み |
