...
データ未指定・読み込み失敗時はコード内で生成する最小フォールバック（1枚）で安全に表示しつつ、外部データファイルとプラグイン型コンポーネントの仕組みにより、コードを直接変更せずにプレゼンテーション内容を柔軟に変更できるようにすることを目的とします。デモ用サンプルはアプリに同梱せず、テンプレートガイドとして `.spkg` で別配布します（[slide-package-distribution.md](./slide-package-distribution.md)）。
...
- データ未指定・読み込み失敗時にコード内で生成する最小フォールバックで安全に表示する
...
- コンポーネント拡張
    ...
    - デフォルトコンポーネントの上書き
- 最小フォールバック表示
    - コード内で生成する最小フォールバック（1枚）を提供
    - データファイルが未指定・読み込み失敗時にフォールバックで表示
- データバリデーション
    - JSONスキーマによるデータ構造の検証
...
requirementDiagram
    ...
    functionalRequirement DefaultTemplate {
        id: FR_400
        text: "データ未指定または読み込み失敗時に、コード内で生成する最小フォールバックを表示すること。デモ用サンプルはアプリに同梱せず `.spkg` として別配布する"
        risk: low
        verifymethod: test
    }
...
requirementDiagram
    ...
    functionalRequirement DefaultData {
        id: FR_401
        text: "最小フォールバックデータをコード内で生成して提供すること"
        risk: low
        verifymethod: test
    }
...
requirementDiagram
    ...
    performanceRequirement BackwardCompatibility {
        id: NFR_102
        text: "最小フォールバック表示時も、本編と同一のレンダラで描画され、見た目・動作に差異がないこと"
        risk: high
        verifymethod: test
    }
...
requirementDiagram
    ...
    performanceRequirement BackwardCompatibility {
        id: NFR_102
        text: "最小フォールバック表示時も本編と同一レンダラで描画されること"
        risk: high
        verifymethod: test
    }
...
スライド資料の内容を、構造化データファイル（JSON）とカスタムデザインコンポーネントを指定することで、ソースコードを直接変更せずに差し替えられること。データファイルが未指定・読み込み失敗時はコード内で生成する最小フォールバック（1枚）を表示し、データファイルの編集のみで資料内容を変更できる仕組みを提供する。
...
外部データファイルが未指定または読み込み失敗時に、コード内で生成する最小フォールバック（1枚）を表示する。デモ用サンプルはアプリに同梱せず、テンプレートガイドとして `.spkg` で別配布する（[slide-package-distribution.md](./slide-package-distribution.md)）。
...
| サブ要求   | 優先度  | 説明                             |
|:-------|:-----|:-------------------------------|
| FR_401 | Must | 最小フォールバックデータをコード内で生成して提供       |
...
### NFR_102: フォールバック表示の一貫性

最小フォールバック表示時も、本編と同一のレンダラ（App / SlideRenderer）で描画され、見た目・動作に差異がないこと。フォールバック専用の描画経路を別途持たないこと。

**優先度:** Must

**検証方法:** テスト — 本編のレンダラ経路と比較
...
- 最小フォールバック表示のために専用のレンダリング経路を追加しないこと（本編と同一のレンダラを使用する）
...
| 最小フォールバック | データ未指定・読み込み失敗時にコード内で生成する1枚のスライドデータ                      |
