...
ホーム画面・プレゼンテーション画面・編集画面（`SlideEditor`）のいずれからも左上に設定ボタン（アイコン）を常時表示する。プレゼンテーション画面ではスライドコンテンツの上にオーバーレイとして配置され、Reveal.jsのスライド操作を妨げない。ホーム画面ではスライドの読み込み中であってもボタンを無効化せず、言語を変更できる状態を保つ。編集画面は独自ツールバーを持つため、ボタンはそのツールバー内の最左に配置し（フローティングのオーバーレイではない）、他画面と同じ「左上」の視覚的位置を保つ。

**#126（2026-07-29）**: 編集画面への導線を追加。ダイアログ本体（`SettingsWindow`）は元々 Root（`RootContent`）が保持しており、編集画面にも `view === 'edit'` のときからマウントされていたため、`SlideEditor` に `onOpenSettings` prop を渡すのみで対応した。
