...
> **取得とキャッシュの詳細な契約**（スキーム制約・展開先の命名・再利用の条件・タイムアウト・配布サンプルの取得）は [slide-package-distribution.md](./slide-package-distribution.md) が所有する。本 PRD は「URL 入力という入口が存在する」ことのみを要求として持つ。
