...
このドキュメントは、React + Reveal.js ベースのスライドプレゼンテーション作成ツール（Tauri製ローカルデスクトップアプリ）の開発を統制する非交渉原則と基準を定義します。すべてのコード、仕様、設計判断はこれらの原則に従う必要があります。
...
**原則**: アプリケーションが表示するプレゼンテーションの視覚的品質と伝達力を最優先とする。
...
- 情報の理解を妨げるレイアウト崩れやアニメーション過多

**準拠例**:

- 明確なビジュアル階層による情報設計
- テーマ・レイアウトシステムに従った一貫した表示

---

### B-002: 同梱・配布コンテンツの正確性

**原則**: アプリに同梱・配布するサンプルやテンプレート（`samples/` 配下）で説明する機能・コマンド・フローは、実際のアプリ機能と一致していなければならない

**適用範囲**: `samples/` 配下のサンプル・テンプレートコンテンツ

**検証方法**:

- [ ] サンプルで説明されている機能がアプリに実際に存在するか
- [ ] サンプル内のコマンド名やフロー図が最新の実装と一致しているか

**違反例**:

- 廃止された機能や存在しないコマンドをサンプルで紹介する
- アプリの実際の仕様と異なるフロー図や成果物例をサンプルに掲載する

**準拠例**:

- アプリの実装（`src/` 等）を確認した上でサンプルのコマンド名・フローを記載する
- アプリの機能変更に合わせてサンプル内容を見直す
...
- スライドコンポーネントが `<section>` でラップせず `<div>` のみを返す
- Reveal.js が付与するクラス名やデータ属性を独自CSSで上書きし機能を破壊する

**準拠例**:

- `SlideRenderer.tsx` の各描画関数が `<section>` 要素を返す
- `.reveal > .slides > section` の構造を壊さず、レイアウトラッパーで内側のみを構成する
...
| **デスクトップシェル** | Tauri（Rust）                            | ローカルファイルアクセス・IPC・ネイティブアプリ配布     |
...
├── theme.ts                # テーマ型定義・デフォルトテーマ
├── addon-bridge.ts         # アドオン向けグローバルAPI公開
├── addonLoader.ts          # ビルトインアドオンの検出・読み込み
├── aiGenerate.ts           # AI連携によるスライドJSON生成
├── sampleSlides.ts         # サンプルスライド取得元の解決（同梱→配布パッケージ→案内）
├── localSlideLoader.ts     # ローカルファイル/.spkgの選択・展開・asset URL変換
├── slidePackageArchive.ts  # .spkgパッケージの読み書き
├── editModeSave.ts         # 編集モードでの保存処理
├── presenterViewEntry.tsx  # 発表者ビュー（別ウィンドウ）のエントリーポイント
├── keyboardTarget.ts       # キーボードショートカットの入力対象判定
├── components/             # UIコンポーネント（SlideRenderer, ComponentRegistry等）
├── layouts/                # レイアウトラッパー（TitleLayout, ContentLayout等）
├── edit/                   # 編集モードUI（SlideEditor, AiGeneratePanel等）
├── hooks/                  # カスタムフック（useReveal, usePresenterView等）
├── data/                   # データローダー・バリデーション・デフォルトデータ
├── i18n/                   # 多言語対応（ロケールリソース読み込み）
├── toast/                  # トースト通知
├── styles/                 # CSSスタイル（global.css, CSS Modules）
└── __screenshot__/         # スクリーンショット/E2E用Tauri IPCモック（dev限定）
```
...
2. **正確性** - 同梱・配布するサンプル/デッキ内容が実際のアプリ機能と一致しているか？
3. **シンプルさ** - 最もシンプルな解決策か？
...
**目的・B-001・B-002を汎用ツールの実態に合わせて再定義（破壊的変更、#134）**

- 目的: 「AI-SDDワークフロー デモプレゼンテーションプロジェクト」から「React + Reveal.js ベースのスライドプレゼンテーション作成ツール」へ再定義。実態（README.md/CLAUDE.md）はAI-SDD固有コードを持たない汎用ツールであり、AI-SDD由来の資産は同梱サンプルのデータのみだったため
- B-001: 「プレゼンテーション品質の優先」→「表示品質の優先」。「AI-SDDワークフローの価値を聴衆に正確に伝える」を目的から削除し、検証項目①（AI-SDDワークフローの価値を正確に伝えているか）を削除。21箇所の既存参照は②③（視覚的品質・1280x720）のみが実際に引用されており、①はすべての参照で無視されていたため
- B-002: 「コンテンツの正確性」→「同梱・配布コンテンツの正確性」。適用範囲を「すべてのスライドコンテンツ」から `samples/` 配下のサンプル・テンプレートに限定し、比較対象を「実際のプラグイン機能」から「実際のアプリ機能」に変更。既存3件の参照のうち2件（`presenter-view.md`, `slide-content-customization_spec.md`/`_design.md`）が条文の趣旨と異なる意味（デッキ内容論・後方互換性）で誤用されていたため
- 意思決定フレームワーク「2. 正確性」を「AI-SDDの仕様と一致しているか」から「同梱・配布するサンプル/デッキ内容が実際のアプリ機能と一致しているか」に変更
- 既存の参照箇所（B-001: 21件、B-002: 3件）を新しい条文の趣旨に合わせて棚卸し（詳細は各ドキュメントの変更を参照）

### v1.1.1 (2026-07-29)

**CONSTITUTION自体の記述整合性を修正**

- A-003・A-005の検証方法を `.spkg` 配布アーキテクチャに整合させた変更（2026-07-28, #110）を本バージョンに反映（当時のバージョン・変更履歴が未更新だった）
- B-002・T-002に違反例・準拠例を追加（他原則との3点セット構成を統一）
- 技術スタック制約に「デスクトップシェル」（Tauri/Rust）を追加
- モジュール構成を現在の `src/` 構造に更新（`edit/`, `i18n/`, `toast/`, `__screenshot__/` 等を追加）
- 節番号の構造を修正（`開発標準`/`アーキテクチャ制約`/`意思決定フレームワーク` を「4. 技術制約」の後ろに移動し、1〜4の原則階層番号と連続させた）
- 「この原則を参照すべきドキュメント」に `PRD_TEMPLATE.md`/`requirement/*.md` を追加
