...
| Reveal.js非干渉           | `stopPropagation()` は実装していない（`src/` 全体に該当コードなし。v1.1.0 時点の記述が `DialogFrame`＝MUI Dialog への移行（v1.3.0）で失効したまま残っていたため訂正）。実際の防御は Reveal 自身の入力ガード（`document.activeElement.tagName` が `INPUT`/`TEXTAREA`、または `isContentEditable` なら無視）のみで、`<select>` 等の非テキスト要素にフォーカスがある間の挙動は未検証。恒久対応は課題として残す |
...
| 設定・ショートカットダイアログが開いている間、Reveal.js のキーボード操作を防ぐ明示的な仕組みがない。Reveal 自身の入力ガードは `INPUT`/`TEXTAREA`/`contentEditable` のみを見るため、ダイアログ内の `<select>`（言語選択）やボタンにフォーカスがある状態でキー入力した場合に Reveal 側へ伝播しないかは未検証（§7 参照） | 中   | ダイアログ本体（`DialogFrame`）の `onKeyDown` で対象外キーのみ `stopPropagation()` するか、Reveal 側で `keyboardCondition` を使い明示的に無効化区間を作る。実装後は `e2e/settings.spec.ts` に非干渉の回帰テストを追加する |
...
**`/constitution validate` で検出した記述と実装の乖離を修正（ドキュメントのみ）:**

- §7「Reveal.js非干渉」の実現方針が `stopPropagation()` としていたが、`src/` に該当コードは存在しない。v1.1.0 時点の記述が `DialogFrame`（MUI Dialog）移行（v1.3.0）で失効したまま残っていたため、実態（Reveal 自身の入力ガードのみで `<select>` 等の非テキスト要素は未検証）へ訂正した
- 上記を §9.3 未解決の課題に追加（影響度: 中。対応方針: `DialogFrame` の `onKeyDown` での抑止、または Reveal の `keyboardCondition` の活用）
