...
[slide-edit-mode.md](../requirement/slide-edit-mode.md)（#13）で用意した編集モード（器）は、`slides.json` の編集・ライブプレビュー・ローカル保存・`.tgz` 書き出しをアプリ内で完結させた。しかしスライドの**生成**は当面「外部（Claude Code 等）に委ねる」方針であり、アプリ単体でプロンプトからスライドを起こす導線はない。非エンジニアが外部ツールを併用せずに下書きを得る手段がまだない。

生成を内蔵するには、ビューワーがこれまで持たなかった 2 つの関心事—**ネットワーク通信**（Anthropic API 呼び出し）と**秘密情報**（API キー）—を導入する必要がある。ビューワーは読み取り中心で外部通信も秘密情報も持たない設計であり、#13 の器も「編集モード時のみ書き込みを有効化する capability 分離」で安全性を担保している。本仕様は、この安全性を損なわずに**生成能力だけを足す**ことを最大の設計課題とする。
...
- **差し込み可能な生成インターフェース**: 生成は「生成器（プロンプト → `slides.json` 候補を返す単位）」として抽象化し、内蔵（Anthropic API）と外部（Claude Code 等）を同一契約で差し替える（**FR-002**）。抽象の切れ目をモデル呼び出しではなく生成器単位に置くことで、外部の完結した生成手段も同一契約を満たせる。
- **ネットワーク・秘密情報のネイティブ境界集約**: Anthropic API 呼び出しと API キー保管は Rust コマンド境界の単一チョークポイントに集約し、内蔵はネイティブ（Rust）側の HTTP 経路で接続する。`fetch` 権限や API キーの生値をフロントエンド（JS）へ渡さない（**DC-002 / NFR-003**）。
...
| FR-002 | 差し込み可能な共通生成インターフェースを介して内蔵（Anthropic API）と外部（Claude Code 等）を切り替える | 必須  | FR-002 / UR-001 |
| FR-003 | 内蔵生成は Rust コマンド境界から Anthropic Messages API を既定モデルで呼び出す                | 必須  | FR-003 / UR-001 / UR-002 / DC-002 |
...
| FR-006 | Anthropic API キーを OS キーチェーンに保管・更新・削除し、生値を JS へ渡さない                  | 必須  | FR-006 / UR-002 / DC-002 |
...
| `src` | `aiGenerate.ts` | `generateSlides(request)` / `cancelGenerate()` / `setApiKey` / `deleteApiKey` / `getApiKeyStatus` / `setGenerationEnabled` | Rust コマンドの呼び出し口。生成/中断（`generateSlides`/`cancelGenerate`）は編集モード かつ 生成有効時のみ成功。キー管理（`setApiKey`/`deleteApiKey`/`getApiKeyStatus`）と `setGenerationEnabled` は編集モードのみ必須（生成有効化前のセットアップ・事前ゲート表示のため）（FR-003/006/007/009/010） |
...
| `src-tauri` | `lib.rs` | `set_api_key` / `delete_api_key` / `has_api_key`（Rust コマンド） | API キーの保管・削除・存在確認。編集モードのみ必須（生成有効は不要＝セットアップ手順）。`has_api_key` は keyring 非アクセスで生値を返さず状態のみ返し、生成無効時も事前ゲート表示のために呼べる（FR-006/007） |
...
```rust
// src-tauri/src/lib.rs — 生成・キー管理コマンド。生成/中断（generate_slides/cancel_generation）は
// 編集モード かつ 生成有効時のみ成功。キー管理（set/delete/has_api_key）と set_generation_enabled は
// 編集モードのみ必須（生成有効は不要）。has_api_key は keyring 非アクセスで状態のみ返す。
// 内部ゲート機構・生成器 trait・HTTP/キーチェーン実装は Design Doc §5/§6 参照）
...
fn has_api_key() -> Result<ApiKeyStatus, String>  // 生値は返さない
```
...
| 用語 | 説明 |
|------|------|
| 内蔵生成 | アプリが Anthropic API を直接呼び出して `slides.json` を生成する方式（`GeneratorKind='builtin-anthropic'`） |
...
function AiGeneratePanel({ onGenerated, onError }: AiGeneratePanelProps) {
  const status = useApiKeyStatus() // FR-007: キー状態を購読
  const [prompt, setPrompt] = useState('')

  const handleGenerate = async () => {
    const result = await generateSlides({ prompt, kind: 'builtin-anthropic' }) // FR-003/009: Rust 境界（編集モード＋生成有効ゲート）
...
- ネットワーク通信と API キー保管は Rust コマンド境界に集約し、内蔵はネイティブ HTTP で Anthropic API へ接続する。`fetch`／秘密権限を JS へ開放しない（DC-002 / NFR-003）。
