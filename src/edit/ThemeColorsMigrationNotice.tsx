...
import { listOverriddenThemeColorKeys, planThemeColorsDelegation } from '../brandMigration'
...
export function ThemeColorsMigrationNotice({ themeColorsPalette, brandColors, onDelegate }: ThemeColorsMigrationNoticeProps) {
  ...
  const plan = brandColors ? planThemeColorsDelegation(themeColorsPalette, brandColors) : undefined
  ...
  return (
    <Alert severity="warning" sx={{ mb: 1 }}>
      <Stack direction="row" spacing={1} alignItems="center" flexWrap="wrap" useFlexGap>
        ...
        <Tooltip title={plan ? '' : t('brandMigration.needsBrandTheme', 'meta.brandTheme を設定すると委譲できます')}>
          <span>
            <Button size="small" variant="outlined" disabled={!plan} onClick={onDelegate}>
